...
class HeadPoseService:
    ...
    def __init__(self):
        ...
        # 🔹 حدود الحركة
        self.yaw_threshold = 60
        self.pitch_threshold = 50
...
#                 matched_student = self._match_face_to_student(face.embedding)

#                 if matched_student:
#                   student_id = matched_student["student_id"]
#                 else:
#                   student_id = f"unknown_{idx}"
...
#                 # 👇 حركة رأس غير طبيعية
#                 if abs(pitch) > self.pitch_threshold:
#                     key = f"head_movement_{student_id}"
#                     if self.start_time[key] is None:
#                         self.start_time[key] = now
#                     elif now - self.start_time[key] >= self.required_seconds["head_movement"]:
#                         last = self.last_reported.get(key, 0)
